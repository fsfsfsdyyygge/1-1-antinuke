...
class AntiNikkiBot(commands.Bot):
    ...
    async def on_ready(self) -> None:
        self.log.info("1/1 ANTINUKE online as %s (%s)", self.user, self.user.id if self.user else "unknown")
    ...
    async def on_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError) -> None:
        ...
        message = "1/1 ANTINUKE could not complete that command. Check its permissions and role position."
