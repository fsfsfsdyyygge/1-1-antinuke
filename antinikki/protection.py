...
class SecurityPanel(discord.ui.View):
    ...
    async def incidents(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        e = discord.Embed(title="Recent 1/1 ANTINUKE Incidents", color=discord.Color.red())
        e.description = "\n".join(
...
class AntiNikki(commands.Cog):
    ...
    antinikki = app_commands.Group(name="antinuke", description="1/1 ANTINUKE anti-nuke protection")
    ...
    async def require_owner(self, interaction: discord.Interaction) -> bool:
        ...
        if not allowed:
            await interaction.response.send_message("Only the Discord server owner or an 1/1 ANTINUKE owner can use this command.", ephemeral=True)
        return allowed
    ...
    async def panel_embed(self, guild: discord.Guild) -> discord.Embed:
        ...
        e = discord.Embed(title="1/1 ANTINUKE Security Panel", color=discord.Color.green() if cfg["enabled"] and not missing else discord.Color.orange())
    ...
    async def config_embed(self, guild: discord.Guild) -> discord.Embed:
        ...
        e = discord.Embed(title="1/1 ANTINUKE Configuration", color=discord.Color.blurple())
        e.description = "\n".join(rows)
    ...
    async def log_incident(self, guild: discord.Guild, actor: discord.Member | None, event: str, action: str, details: dict[str, Any]) -> None:
        ...
        if isinstance(channel, discord.TextChannel):
            e = discord.Embed(title="1/1 ANTINUKE Security Incident", color=discord.Color.red(), timestamp=discord.utils.utcnow())
    ...
    async def respond(self, guild: discord.Guild, actor: discord.Member, event: str, cfg: dict[str, Any]) -> None:
        ...
        try:
            if action == "ban":
                await actor.ban(reason=f"1/1 ANTINUKE triggered: {event}")
            elif action == "kick":
                await actor.kick(reason=f"1/1 ANTINUKE triggered: {event}")
            elif action == "timeout":
                await actor.timeout(discord.utils.utcnow() + dt.timedelta(hours=24), reason=f"1/1 ANTINUKE triggered: {event}")
                ...
                if removable:
                    await actor.remove_roles(*removable, reason=f"1/1 ANTINUKE triggered: {event}")
            elif action != "log_only":
                action = "log_only"
        except discord.HTTPException as exc:
            action = f"failed_{action}"
            self.log.exception("Response failed")
        locked = await self.lockdown(guild, cfg, f"1/1 ANTINUKE emergency lockdown: {event}") if cfg.get("lockdown_on_trigger") else 0
    ...
    async def setup(self, interaction: discord.Interaction, log_channel: discord.TextChannel) -> None:
        ...
        await interaction.response.send_message("1/1 ANTINUKE is enabled. Review `/antinuke status` and place its role above every role it must secure.", ephemeral=True)
    ...
    @antinikki.command(name="panel", description="Open the 1/1 ANTINUKE security panel")
    ...
    @antinikki.command(name="enabled", description="Enable or disable 1/1 ANTINUKE protection")
    async def enabled(self, interaction: discord.Interaction, value: bool) -> None:
        ...
        await self.bot.db.set(interaction.guild_id, cfg)
        await interaction.response.send_message(f"1/1 ANTINUKE protection is now **{'enabled' if value else 'disabled'}**.", ephemeral=True)

    @antinikki.command(name="incidents", description="Show recent 1/1 ANTINUKE security incidents")
    ...
    async def lockdown_command(self, interaction: discord.Interaction) -> None:
        ...
        cfg = await self.config(interaction.guild_id)
        count = await self.lockdown(interaction.guild, cfg, f"Manual 1/1 ANTINUKE lockdown by {interaction.user}")
    ...
    @antinikki.command(name="unlock", description="Restore permissions saved by 1/1 ANTINUKE lockdown")
    async def unlock(self, interaction: discord.Interaction) -> None:
        ...
        for item in cfg.get("lockdown_roles", []):
            ...
            try:
                await role.edit(permissions=discord.Permissions(int(item["permissions"])), reason=f"1/1 ANTINUKE unlock by {interaction.user}")
                restored += 1
    ...
    async def prefix_change(self, ctx: commands.Context, new_prefix: str | None = None) -> None:
        ...
        if not isinstance(ctx.author, discord.Member) or not self.owner_member(ctx.author):
            await ctx.reply("Only the server owner or an 1/1 ANTINUKE owner can change the prefix.", mention_author=False)
            return
        cfg = await self.config(ctx.guild.id)
    ...
    async def prefix_an(self, ctx: commands.Context) -> None:
        """Short 1/1 ANTINUKE prefix command group."""
        if not isinstance(ctx.author, discord.Member) or not await self.security_admin(ctx.author):
    ...
    async def prefix_an_lockdown(self, ctx: commands.Context) -> None:
        ...
        cfg = await self.config(ctx.guild.id)
        count = await self.lockdown(ctx.guild, cfg, f"Manual 1/1 ANTINUKE lockdown by {ctx.author}")
    ...
    async def prefix_an_enable(self, ctx: commands.Context) -> None:
        ...
        await ctx.reply("1/1 ANTINUKE protection enabled.", mention_author=False)
    ...
    async def prefix_an_disable(self, ctx: commands.Context) -> None:
        ...
        await ctx.reply("1/1 ANTINUKE protection disabled.", mention_author=False)
    ...
    async def prefix_an_help(self, ctx: commands.Context) -> None:
        """Display the full 1/1 ANTINUKE command reference."""
        if not isinstance(ctx.author, discord.Member) or not await self.security_admin(ctx.author): return
    ...
    async def owner_psw_give(self, ctx: commands.Context, member: discord.Member, role: discord.Role) -> None:
        ...
        if role.is_default() or role.managed:
            await ctx.author.send(f"1/1 ANTINUKE cannot assign the managed/default role **{role.name}** in **{ctx.guild.name}**.")
            return
        if me is None or role >= me.top_role:
            await ctx.author.send(f"Move 1/1 ANTINUKE's role above **{role.name}** in **{ctx.guild.name}**, then try again.")
            return
        try:
            await member.add_roles(role, reason=f"1/1 ANTINUKE OWNER_IDS grant by {ctx.author} ({ctx.author.id})")
        except discord.HTTPException as exc:
